...
export default function Card({ changeToogle, refreshTable }: props) {
  ...
  async function searchMachine(codigo: string) {
    ...
    await Api.get(`/maquina/${codigo}`)
      .then((res) => {
        ...
        setInfoMachine((current) => ({
          ...current,
          codigo: res.data.codigo,
          descricao: res.data.descricaoMaquina,
        }));
      })
      .catch((err) => {
  ...
  return (
    <div className={style.card}>
      ...
      <div className={tooglFilter ? style.filterPeca : style.filterPeca_close}>
        <CardPecas />
      </div>
      <div
        className={
          toogleFilterMaquina ? style.filterMachine : style.filterMachine_close
        }
      >
        <FormFilterMachine
      ...
      <section className={style.body}>
        <div className={style.container_codigo}>
          <Input
            ...
            iconLeft={{
              action: () =>
                setToogleFiterMaquina((current) => (current = true)),
              icon: Icons.Filter,
            }}
          />
        </div>
